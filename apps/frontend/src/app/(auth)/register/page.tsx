...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    // Client-side: kiểm tra mật khẩu trùng khớp trước khi gọi API
      ...
      <div className="w-full max-w-lg p-8 rounded-3xl bg-white border border-[#E8E8E8] shadow-xl space-y-6">
        ...
        {successInfo ? (
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-xs font-semibold text-[#2C2C2C]">Xác Nhận Mật Khẩu</label>
              <div className="relative">
                <Lock className="w-4 h-4 text-gray-400 absolute left-3.5 top-3.5" />
                <input
                  type="password"
                  ...
                  className={`w-full bg-[#F9F9F9] border ${
                    passwordError ? 'border-red-400 focus:border-red-500 focus:ring-red-500/20' : 'border-[#E8E8E8] focus:border-[#E03C3D] focus:ring-[#E03C3D]/20'
                  } focus:ring-1 rounded-xl pl-10 pr-4 py-2.5 text-sm text-[#2C2C2C] focus:outline-none transition-all placeholder-gray-400`}
                />
              </div>
              {passwordError && (
